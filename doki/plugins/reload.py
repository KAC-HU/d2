...
async def reload_admin_cache(client, message: Message, _):
    try:
        ...
        await message.reply_text(
            "Failed to Refresh Admins List, Make Sure Uh Promoted the Bot."
        )
...
async def restartbot(client, message: Message, _):
    ...
    return await mystic.edit_text(
        f"Successfully Rebooted {MUSIC_BOT_NAME} For Ur Chats, Now Uh Can Start Playing Again..."
    )
...
async def stop_download(client, CallbackQuery: CallbackQuery, _):
    ...
    if not task.done():
        try:
            ...
            return await CallbackQuery.answer(
                "Failed to Cancelled Downloading...", show_alert=True
            )
    await CallbackQuery.answer(
        "Failed to Recognized The Ongoing task.", show_alert=True
    )
